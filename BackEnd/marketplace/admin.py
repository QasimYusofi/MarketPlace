...
class UserAdmin(BaseUserAdmin):
    model = User
    list_display = (
        'id', 'phone', 'first_name', 'last_name', 'email', 'role', 'status', 'is_verified', 'is_staff', 'created_at'
    )
    ...
    fieldsets = (
        ...
        ('Status', {'fields': ('role', 'status', 'is_verified', 'last_login', 'created_at', 'updated_at')}),
    )

    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('phone', 'password1', 'password2', 'first_name', 'last_name', 'email', 'role', 'status', 'is_verified', 'is_staff', 'is_superuser'),
        }),
    )
